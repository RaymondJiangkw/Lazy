 ...
 */
func (d *Display) ProgressBar(options *ProgressBarOption) error {
...
func (d *Display) EasyProgress(writer io.Writer, prefix string, postfix string, maximum int, signal <-chan struct{}) error {
	var text string
	var cnt int
...
func (d *Display) TemporaryText(writer io.Writer, text string, signal <-chan struct{}) {
	fmt.Fprintf(writer, "%s", text)
	<-signal
	fmt.Fprintf(writer, "%s", d.generator.Flush(len(text)))
}
